...
from langchain_gemini import GeminiEmbeddings, ChatGemini
import requests
import os
...
# Function to fetch vector store from URL
...
# Function to create retriever for context
def get_context_retriever_chain(vector_store):
    llm = ChatGemini()

    retriever = vector_store.as_retriever()

    def retriever_func(prompt):
        response = requests.post(
...
def get_conversational_rag_chain(retriever_func):
    def conversation_chain(input, chat_history):
        prompt = f"Given the following conversation history:\n{chat_history}\nAnswer the question: {input}"
        return retriever_func(prompt)

    return conversation_chain

# Function to get response based on user input
def get_response(user_input):
    retriever_func = get_context_retriever_chain(st.session_state.vector_store)
    conversation_chain = get_conversational_rag_chain(retriever_func)
    
    response = conversation_chain(user_input, st.session_state.chat_history)
...
if not website_url:
    ...
    for message in st.session_state.chat_history:
        if isinstance(message, AIMessage):
            with st.container():
                st.text_area("AI Response", message.content, height=100)
        elif isinstance(message, HumanMessage):
            with st.container():
                st.text_area("User Input", message.content, height=100)
